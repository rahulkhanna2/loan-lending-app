...
public class LoanService {
    ...
    public void acceptLoan(final long loanApplicationId, final String lenderUsername) {
        ...
        LoanApplication application = findLoanApplication(loanApplicationId);
        User borrower = application.getBorrower();
        Money money = application.getAmount();
        lender.withdraw(money);
        borrower.topUp(money);
        loanRepository.save(new Loan(lender, application));
    ...
    public void repayLoan(final Money amountToRepay, final long loanId, final User borrower) {
        ...
        Money actualPaidAmount  = amountToRepay.getAmount() > loan.getAmountOwed().getAmount() ? loan.getAmountOwed() : amountToRepay;
